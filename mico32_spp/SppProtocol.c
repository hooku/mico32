...
#ifdef MICO32
    char ping_request   [] = { "Show your self!"        };
    char ping_reply     [] = { "I am MICO on CC3200"    };
    if ((g_client_fd != 0xFFFF) && (strncmp(inBuf, ping_request, sizeof(ping_request) - 1) == 0))
    {
        SocketSend(g_client_fd, ping_reply, sizeof(ping_reply));
    }
#endif // MICO32
...
OSStatus sppUartCommandProcess(uint8_t *inBuf, int inLen, mico_Context_t * const inContext)
{
  ...
  struct sockaddr_t addr;
#ifdef MICO32
    if (g_client_fd != 0xFFFF)
    {
        SocketSend(g_client_fd, inBuf, inLen);
    }
#else // MICO32
  addr.s_ip = IPADDR_LOOPBACK;
